...
description: "Prepare and audit local, Slurm, PBS and cloud/HPC computational-chemistry execution: environment inspection, dependency checks, CPU/GPU and native-code acceleration planning, resource estimates, job scripts, batch DAGs, monitoring, failure classification, checkpoint/restart policy, provenance, reproducibility and scientific CI."
license: MIT
...
4. Estimate CPU/GPU hours, memory, wall time and storage before production submission.
5. Generate scripts from a reviewed manifest. Submission, cancellation and destructive cleanup require explicit user instruction.
6. Monitor scheduler and logs. Classify exact failures before changing one parameter at a time.
7. Preserve input, actual script, stdout/stderr, checkpoints, output hashes, software/environment fingerprint and restart lineage.
8. Promote outputs only after the engine parser validates them; HPC success is not scientific acceptance.
...
`plan_acceleration.py` emits a benchmark and compatibility plan; it never patches, launches or claims a faster DFT engine. Submission remains approval-gated.
