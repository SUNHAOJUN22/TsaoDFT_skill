...
def parse(path: Path) -> dict:
    ...
    version = (
        re.search(r"CP2K\| version string:\s*(.+)", text).group(1).strip()
        if re.search(r"CP2K\| version string:\s*(.+)", text)
        else None
    )
    energies = [float(x) for x in re.findall(rf"ENERGY\| Total FORCE_EVAL.*?energy \(a\.u\.\):\s*({FLOAT})", text)]
    ...
    max_force = [float(x) for x in re.findall(rf"Max\. gradient\s*=\s*({FLOAT})", text)]
    warnings = []
    ...
    return {
        ...
        "program_ended": ended,
        "version": version,
        "last_total_energy_hartree": energies[-1] if energies else None,
        "last_total_energy_eV": energies[-1] * HARTREE_TO_EV if energies else None,
        "energy_count": len(energies),
        "scf_converged": scf_converged,
        "geometry_converged": geo_converged,
        "last_max_gradient": max_force[-1] if max_force else None,
        "warnings": warnings,
...
def main() -> int:
    ...
    return 0 if r["program_ended"] else 1
