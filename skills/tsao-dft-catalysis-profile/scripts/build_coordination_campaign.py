...
def main() -> int:
    ...
    a = ap.parse_args()
    d = yaml.safe_load(a.campaign.read_text()) or {}
    axes = d.get("axes") or {}
    missing = [k for k in REQUIRED_AXES if not axes.get(k)]
    if missing:
        print(json.dumps({"ok": False, "errors": [f"missing/nonempty axes: {missing}"]}, indent=2))
        return 1
    names = list(axes)
    ...
    for combo in itertools.product(*(axes[n] for n in names)):
        ...
        rows.append(
            {
                "candidate_id": f"{d.get('campaign_id', 'CAT')}-{len(rows) + 1:04d}",
                **row,
    ...
    if limit and len(rows) > limit:
        print(
            json.dumps({"ok": False, "errors": [f"campaign expands to {len(rows)} > max_candidates {limit}"]}, indent=2)
        )
        return 2
    ...
    with a.out.open("w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        w.writerows(rows)
    print(json.dumps({"ok": True, "candidate_count": len(rows), "out": str(a.out)}, indent=2))
    return 0
