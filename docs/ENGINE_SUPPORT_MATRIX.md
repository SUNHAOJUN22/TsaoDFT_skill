...
| Real benchmark evidence and qualification | L2 validated adapter; scoped L3 eligibility output | multi-format import, hash/identity/parser gates, numerical equivalence, robust statistics, immutable bundle and explicit qualification states | repository fixtures do not establish a real speedup; scoped qualification requires real-engine artifacts and independent review |
...
A user or lab may promote a capability to L3 only by adding immutable regression artifacts, engine/version/build/hardware records, expected-value tolerances, sufficient repeats, numerical-equivalence evidence, artifact hashes, independent review and site information without credentials or licensed files.
