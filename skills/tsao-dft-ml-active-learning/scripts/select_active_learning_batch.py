...
ap.add_argument("--group", default="parent_id")
ap.add_argument("--out", type=Path, required=True)
a = ap.parse_args()
rows = list(csv.DictReader(a.pool.open(encoding="utf-8")))
rows.sort(key=lambda r: (-float(r[a.uncertainty]), r.get(a.group, ""), r.get("sample_id", "")))
sel = []
seen = set()
for r in rows:
